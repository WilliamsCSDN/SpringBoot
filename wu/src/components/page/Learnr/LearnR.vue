...
<!--导航-->
        ...
        <div style="width: 100%;height: 500px;padding-top: 150px;">
            <div class="aaa">
                <span style="color:white">学习路线</span>
            </div>
...
//                window.location.reload()
            ...
            search1(){
                this.$ajax.get("getlearncontent").then(res => {
                    var a=0;
                    for(var i=0;i<res.data.length;i++){
                        a+=res.data[i].learnContent.length;
                    }
                    this.total=a;
                })
                this.$ajax.get("getlearncontent?start=0&num=5").then(res => {
                    this.search2 = res.data;
                })
            },
            currentchange(cpage){
                var url="/getlearncontent?start="+(cpage-1)*5+"&num=5";
