...
public class LearnContentController {
    ...
    public Object selectAll(String id, String start, String num) {
        ...
        if(learnContents != null) {
            logger.info("用户 : {} 查询学习线路ID{}成功", UserInterceptor.SESSION_USER,id);
            return learnContents;
        }
        else {
            logger.error("用户 : {} 查询学习线路ID{}失败", UserInterceptor.SESSION_USER,id);
            return "学习线路失败";
        }
    }
    ...
    public Object selectHot(String id) {
        ...
        if(learnContents != null) {
            logger.info("用户 : {} 查询热点学习线路ID{}成功", UserInterceptor.SESSION_USER,id);
            return learnContents;
        }
        else {
            logger.error("用户 : {} 查询热点学习线路ID{}失败", UserInterceptor.SESSION_USER,id);
            return "学习线路不存在";
        }
    }
    ...
    public Object getrouterbyid(String id){
        if(id != null) {
            ...
            if(learnRouter1s != null) {
                logger.info("用户 : {} 查询学习线路内容ID{}成功", UserInterceptor.SESSION_USER,id);
                return learnRouter1s;
            }
            else {
                logger.error("用户 : {} 查询学习线路内容ID{}失败", UserInterceptor.SESSION_USER,id);
                return "线路没有内容";
            }
        }else return "参数缺失";
    }
    ...
    public Object getlearncontent(String id,String author){
        ...
        if(learnContents != null) {
            logger.info("用户 : {} 查询学习线路内容ID{}作者{}成功", UserInterceptor.SESSION_USER,id,author);
            return learnContents;
        }
        else {
            logger.error("用户 : {} 查询学习线路内容ID{}作者{}失败", UserInterceptor.SESSION_USER,id,author);
            return "没有该线路";
        }
    }
    ...
    public String deletelearncontent(String id){
        if(id != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 删除学习线路ID{}成功", UserInterceptor.SESSION_USER,id);
                return "删除学习线路成功";
            }
            else {
                logger.error("用户 : {} 删除学习线路ID{}失败", UserInterceptor.SESSION_USER,id);
                return "删除学习线路失败";
            }
        }else return "参数缺失";
    }
    ...
    public String insertlearncontent(LearnContent learnContent){
        if(learnContent != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 添加学习线路成功", UserInterceptor.SESSION_USER);
                return "添加学习线路成功";
            }
            else {
                logger.error("用户 : {} 添加学习线路失败", UserInterceptor.SESSION_USER);
                return "添加学习线路失败";
            }
        }else return "参数缺失";
    }
    ...
    public String updatelearncontent(LearnContent learnContent){
        if(learnContent != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 修改学习线路ID{}成功", UserInterceptor.SESSION_USER,learnContent.getId());
                return "修改学习线路成功";
            }
            else {
                logger.error("用户 : {} 修改学习线路ID{}失败", UserInterceptor.SESSION_USER,learnContent.getId());
                return "修改学习线路失败";
            }
        }else return "参数缺失";
    }
    ...
    public String insertlearnrouter(LearnRouter1 learnRouter1){
        if(learnRouter1 != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 添加线路内容成功", UserInterceptor.SESSION_USER);
                return "添加线路内容成功";
            }
            else {
                logger.error("用户 : {} 添加线路内容失败", UserInterceptor.SESSION_USER);
                return "添加线路内容失败";
            }
        }else return "参数缺失";
    }
    ...
    public String deletelearnrouter(String id){
        if(id != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 删除线路内容ID{}成功", UserInterceptor.SESSION_USER,id);
                return "删除线路内容成功";
            }
            else {
                logger.error("用户 : {} 删除线路内容ID{}失败", UserInterceptor.SESSION_USER,id);
                return "删除线路内容失败";
            }
        }else return "参数缺失";
    }
    ...
    public String updatelearnrouter(LearnRouter1 learnRouter1){
        if(learnRouter1 != null) {
            ...
            if (a > 0) {
                logger.info("用户 : {} 修改线路内容ID{}成功", UserInterceptor.SESSION_USER,learnRouter1.getId());
                return "修改线路内容成功";
            }
            else {
                logger.error("用户 : {} 修改线路内容ID{}失败", UserInterceptor.SESSION_USER,learnRouter1.getId());
                return "修改线路内容失败";
            }
        }else return "参数缺失";
    }
    ...
    public void getStreamData(HttpServletResponse response, String id, String zj) {
        ...
        try {
            ...
            logger.info("用户 :{} 获取视频成功",UserInterceptor.SESSION_USER);
        } catch (Exception e) {
            logger.error("用户 :{} 获取视频失败{}",UserInterceptor.SESSION_USER,e);
        }
...
//                    String realPath = request.getSession().getServletContext().getRealPath("/");
                    ...
                    logger.info("用户 :{} 上传视频成功",UserInterceptor.SESSION_USER);
                    return trueFileName;
                }else {
                    logger.error("用户 :{} 上传视频失败",UserInterceptor.SESSION_USER);
                    return "error";
                }
            }else {
                logger.error("用户 :{} 上传视频失败",UserInterceptor.SESSION_USER);
    ...
    public ResponseEntity<byte[]> getImage(HttpServletResponse response, String a){
        ...
        byte[] imageContent ;
        String path = "E:\\bs\\image\\"+cc+"\\"+a;
        try {
            imageContent = fileToByte(new File(path));
            final HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.IMAGE_PNG);
            logger.info("用户 :{} 获取图片成功",UserInterceptor.SESSION_USER);
            return new ResponseEntity<>(imageContent, headers, HttpStatus.OK);
        } catch (Exception e) {
//            e.printStackTrace();
            logger.error("用户 :{} 获取图片失败{}",UserInterceptor.SESSION_USER,e);
        }
        return null;
    }
...
//                e.printStackTrace();
        ...
        logger.error("用户 :{} 获取图片失败",UserInterceptor.SESSION_USER);
        return "图片上传失败";
    }
